...
test('boots combat depth beneath the canonical global UI runtime', async ({ page }) => {
  ...
  expect(snapshot.releaseVersion).toBe('3.12.1-health-readability');
...
test('full momentum activates bounded overdrive with temporary runtime bonuses', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    game.momentum = 96;
    game.addMomentum(10);
    return { snapshot: game.getSnapshot(), baseRecall, activeRecall: game.stack('magnetic-recall'), baseDash, activeDash: game.stack('quick-dash') };
  });
