...
test('release metadata exposes one canonical identity', () => {
  assert.equal(RELEASE_VERSION, '3.4.0-combat-juice');
  assert.equal(RELEASE_LABEL, 'v3.4.0-combat-juice');
  assert.equal(RELEASE_CACHE_NAME, 'one-bullet-arena-v3.4.0-combat-juice');
  assert.equal(RELEASE_INFO.schemaVersion, 1);
  assert.equal(RELEASE_INFO.channel, 'combat-feel-and-juice');
...
test('package, runtime, and service worker consume canonical release metadata', async () => {
  ...
  const combatJuiceRuntimeSource = await readFile(new URL('../src/core/combat-juice-runtime.js', import.meta.url), 'utf8');
  ...
  assert.match(combatJuiceRuntimeSource, /OneBulletVisualOverhaulRuntime/);
